...
init = {
    ...
    'Santa': {'time': 0, 'timeStep': 0.01, 'x': 0, 'y': 1,   'vx': 1,   'vy': 0},
}
...
def propagate(agentId, universe):
    ...
    if agentId == 'Planet':
        ...
        px, py = universe['Satellite']['x'], universe['Satellite']['y']
        # print (F"{px}, {py}")
        dx = px - x
        dy = py - y
        if dx==0: dx = 0.01 # hack
        if dy==0: dy = 0.01 # hack
...
for _ in range(2000):
    ...
    for agentId in init:
        ...
        if set(universe) == set(init):
            ...
            # print(store[t, newState['time']])
        # else:
        #     assert (False)

with open('./public/3body.json', 'w') as f:
